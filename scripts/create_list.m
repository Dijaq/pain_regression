...
identity_list = dir('C:\datasets\Pain\Images');
...
for i=1:length(identity_list)
    ...
    subset_list = dir(fullfile('C:\datasets\Pain\Images',identity_list(i).name));
    ...
    for j=1:length(subset_list)
        ...
        image_list = dir(fullfile('C:\datasets\Pain\Images',identity_list(i).name, subset_list(j).name));
        image_list = image_list(3:end);
        %disp([fullfile('./Images',identity_list(i).name, subset_list(j).name) ' ' num2str(length(image_list)) ' files']);
        disp([fullfile('C:\datasets\Pain\Images',identity_list(i).name, subset_list(j).name) ' ' num2str(length(image_list)) ' files']);
...
%             assert(size(image,1)==112 && size(image,2)==96);
            ...
            label_file = fullfile('C:\datasets\Pain\Frame_Labels\PSPI',identity_list(i).name, subset_list(j).name, [b '_facs.txt']);
