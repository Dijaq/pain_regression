...
for p = 1:total_person
    ...
    for i=1:length(C{1})
        if person_id(i) == p
            ...
            fprintf(list_val, '%s %d\r\n', C{1}{i}, C{2}(i));
        else
            fprintf(list_train, '%s %d\r\n', C{1}{i}, C{2}(i));
        end;
    end;
    fclose(list_train);
    fclose(list_val);
    resample_train_list(fullfile(subfolder,num2str(p),'list_train.txt'), fullfile(subfolder,num2str(p),'list_train_resample.txt'), 1);
